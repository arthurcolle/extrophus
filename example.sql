...
{:ok, result} = Postgrex.Connection.query(pid, "select users.id, users.name, users.loc as loc, earth_distance(ll_to_earth(39.056270, -77.129953), ll_to_earth(loc[0], loc[1])) as distance from users order by distance ASC;")
